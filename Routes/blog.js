...
router.get("/blog/:slug", auth, async (req, res) => {
  try {
...
router.post('/edit-blog/:id', auth, async (req, res) => {
  try {
    const blogId = req.params.id;
...
router.get('/blog/preview/:id', auth, async (req, res) => {
  try {
    const blogId = req.params.id;
...
/*
router.get("/see-blog", auth, async (req, res) => {
  try {
    ...
    const relatedBlogs = blogs.slice(0, 5);
    ...
    // ⚠️ Always pass fallback values to avoid EJS crashes
    return res.render("see-blog", {
      title: "My Blogs",
      blogs: [],
      currentPage: 1,
...
router.get("/see-blog", auth, async (req, res) => {
  try {
    ...
    const query = {}; // Add { author: req.user.id } if you want user-specific blogs
...
router.post("/add-blog",auth,async (req,res)=>{
    const {title, auther,date, expert,content,image,slug,published }= req.body
